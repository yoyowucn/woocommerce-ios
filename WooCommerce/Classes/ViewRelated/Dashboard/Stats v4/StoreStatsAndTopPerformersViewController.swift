...
private extension StoreStatsAndTopPerformersViewController {
    func syncAllStats(onCompletion: ((Error?) -> Void)? = nil) {
        ...
        // On the other hand, when syncing the stats data with the API, we want to use the device time zone to find the time range since the API dates are
        // relative to the site time zone (we use the device time zone to find the time range e.g. 12:00am-11:59pm for "Today" tab).
